...
if(!$this->contracted_rates){
    echo '<div class="error">
        <p>'.__('Label Printing is only available for Contracted Account.', 'wf-shipping-auspost').'</p>
    </div>';
}
...
<table id="auspost_label_settings_table">
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Enable/Disable','wf-shipping-auspost') ?></label>
		</td>
		<td scope="row" class="titledesc" style="display: block;margin-bottom: 20px;margin-top: 3px;">
		
		<fieldset style="padding:3px;">
		  <input class="input-text regular-input " type="checkbox" name="wf_australia_post_email_tracking" id="wf_australia_post_email_tracking" style="" value="yes" <?php echo (isset($general_settings['email_tracking']) && $general_settings['email_tracking'] ==='yes') ? 'checked' : ''; ?> placeholder="">  <?php _e('Email Tracking','wf-shipping-auspost') ?> <span class="woocommerce-help-tip" data-tip="<?php _e('On enabling this, Australia Post API would be able to email the tracking information to the customer.','wf-shipping-auspost') ?>" ></span>
		</fieldset>
		<fieldset style="padding:3px;">
		  <input class="input-text regular-input " type="checkbox" name="wf_australia_post_dir_download" id="wf_australia_post_dir_download" style="" value="yes" <?php echo (isset($general_settings['dir_download']) && $general_settings['dir_download'] ==='yes') ? 'checked' : ''; ?> placeholder="">  <?php _e('Enable Direct Download','wf-shipping-auspost') ?> <span class="woocommerce-help-tip" data-tip="<?php _e('By choosing this option, label and order summary will be downloaded instead of opening in a new browser window.','wf-shipping-auspost') ?>" ></span>
		</fieldset>
        <fieldset style="padding:3px;">
            <input class="input-text regular-input " type="checkbox" name="option_download_labels_auspost_elex" id="option_download_labels_auspost_elex" value="yes" <?php echo (isset($general_settings['save_labels']) && $general_settings['save_labels'] ==='yes') ? 'checked' : ''; ?> placeholder="">  <?php _e('Save Shipping Labels','wf-shipping-auspost') ?> <span class="woocommerce-help-tip" data-tip="<?php _e('By choosing this option, you can store the generated shipping label in the wp-content folder. This will help you access the labels in the future even if it is not available on the shipping carrier API.','wf-shipping-auspost') ?>" ></span>
        </fieldset>
        <fieldset style="padding:3px;" id="wf_australia_post_shipment_label_type_fieldset">
            <input class="input-text regular-input " type="checkbox" name="wf_australia_post_shipment_label_type" id="wf_australia_post_shipment_label_type" style="" value="yes" <?php echo (isset($general_settings['shipment_label_type']) && $general_settings['shipment_label_type'] ==='branded') ? 'checked' : ''; ?> placeholder="">  <?php _e('Show Australia Post Logo on the Shipment labels','wf-shipping-auspost') ?> <span class="woocommerce-help-tip" data-tip="<?php _e('Using this option, you can opt to show or not to show the Australia Post logo on the Shipment labels (Available only for Australia Post).','wf-shipping-auspost') ?>" ></span>
        </fieldset>
		</td>
	</tr>
    <tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Import reference number (IOSS)','wf-shipping-auspost') ?></label>
		</td>
		<td>
			<fieldset style="padding:3px;">
				<input class="input-text regular-input " type="text" name="elex_australia_post_import_reference_number" id="elex_australia_post_import_reference_number" style="" value="<?php echo (!empty($general_settings['import_reference_number'])) ? $general_settings['import_reference_number'] :''; ?>" placeholder='Import Reference Number'> <span class="woocommerce-help-tip" data-tip="<?php _e('Provide here Import reference number (IOSS).','wf-shipping-auspost') ?>" ></span>
			</fieldset>
		</td>
	</tr>
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Customer Address Type (AusPost eParcel)','wf-shipping-auspost') ?></label><span class="woocommerce-help-tip" data-tip="<?php _e('Select customer type (Available only for Australia Post).','wf-shipping-auspost') ?>" ></span>
		</td>
		<td>
		<fieldset style="padding:3px;">
		<?php 
            ...
            if(isset($customer_type) && !empty($customer_type)){
                foreach ($customer_type as $key => $value) {
                    ...
                    {
                        echo '<input class="input-text regular-input " type="radio" name="wf_australia_post_cus_type" id="wf_australia_post_cus_type" style="" value="'.$key.'" checked=true placeholder=""> '.$value.' <br>';
                    }
                    else
                    {
                        echo '<input class="input-text regular-input " type="radio" name="wf_australia_post_cus_type" id="wf_australia_post_cus_type" style="" value="'.$key.'"  placeholder=""> '.$value.' <br>';
                    }
                }
            }
		?>
    ...
    <?php 
        if($this->contracted_rates){ ?>
            <tr valign="top" >
                <td style="width:50%;font-weight:800;">
                    <label for="wf_australia_post_"><?php _e('Bulk Shipment','wf-shipping-auspost') ?></label>
                </td>
                <td>
                    <?php if($this->rate_type == 'startrack'){ ?>
                        <fieldset style="padding:3px;">
                            <?php _e('Default Shipment Service (StarTrack)', 'wf-shipping-auspost');?><span class="woocommerce-help-tip" data-tip="<?php _e('Choose the default service for the shipment which will be set while generating bulk shipment label from order admin page. The default service will be applicable if there is no StarTrack service chosen during the checkout process', 'wf-shipping-auspost')?>"></span><br>
                            <select name="starTrack_default_shipment_service" id="starTrack_default_shipment_service" style="width:200px;">
                                <?php
                                if($is_user_account_contract_type == 'yes'){
                                    if(isset($general_settings['starTrack_default_shipment_service']) && ($general_settings['starTrack_default_shipment_service'] == 'none')){
                                    ?>    <option value="none" selected="selected"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                    }else{
                                    ?>    <option value="none"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                    }
                                        ...
                                        foreach($starTrack_contracted_postage_products as $postage_product){
                                            if(isset($general_settings['starTrack_default_shipment_service'])){
                                                ...
                                                if($postage_product['product_id'] == $general_settings['starTrack_default_shipment_service']){?>
                                                    <option value="<?php echo $postage_product['product_id'];?>" selected="selected"><?php _e( $postage_product['type'], 'wf-shipping-auspost');?></option>
                                                <?php   }else{ ?>
                                                        <option value="<?php echo $postage_product['product_id'];?>"><?php _e( $postage_product['type'], 'wf-shipping-auspost');?></option>
                                                <?php   }
                                            }else{
                                                ?><option value="<?php echo $postage_product['product_id'];?>" selected="selected"><?php _e( $postage_product['type'], 'wf-shipping-auspost');?></option><?php
                                            }
                                        }
                                    }
                                }?>
                    ...
                    <?php }?>
                    <fieldset style="padding:3px;">
                        <?php _e('Default Domestic Service (AusPost)', 'wf-shipping-auspost');?><span class="woocommerce-help-tip" data-tip="<?php _e('Choose the default service for domestic shipment which will be set while generating bulk shipment label from order admin page. The default service will be applicable if there is no Australia Post service chosen during the checkout process', 'wf-shipping-auspost')?>"></span><br>
                        <select name="auspost_default_domestic_shipment_service" id="auspost_default_domestic_shipment_service" style="width:200px;">
                            <?php
                            if($is_user_account_contract_type == 'yes'){
                                if(isset($general_settings['auspost_default_domestic_shipment_service']) && ($general_settings['auspost_default_domestic_shipment_service'] == 'none')){
                                ?>    <option value="none" selected="selected"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                }else{
                                ?>    <option value="none"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                }
                    ...
                    <fieldset style="padding:3px;">
                        <?php _e('Default International Service (AusPost)', 'wf-shipping-auspost');?><span class="woocommerce-help-tip" data-tip="<?php _e('Choose the default service for International shipment which will be set while generating bulk shipment label from order admin page. The default service will be applicable if there is no Australia Post service chosen during the checkout process', 'wf-shipping-auspost')?>"></span><br>
                        <select name="auspost_default_international_shipment_service" id="auspost_default_international_shipment_service" style="width:200px;">
                            <?php
                            if($is_user_account_contract_type == 'yes'){
                                if(isset($general_settings['auspost_default_international_shipment_service']) && ($general_settings['auspost_default_international_shipment_service'] == 'none')){
                                ?>    <option value="none" selected="selected"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                }else{
                                ?>    <option value="none"><?php _e( 'NONE', 'wf-shipping-auspost');?></option><?php
                                }
 ...
 <?php  }
	...
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Documents Output Type','wf-shipping-auspost') ?></label>
		</td>
		<td>
			<fieldset style="padding:3px;">
				<b>( PDF )</b><br/>
    ...
    <tr valign="top" >
        <td style="width:50%;font-weight:800;">
            <label for="wf_australia_post_"><?php _e('Documents Layout','wf-shipping-auspost') ?></label>
        </td>
        <td>
            <fieldset style="padding:3px;">
                <table>
                    <?php
                        if($this->rate_type == 'startrack'){ ?>
                            <tr>
                                            ...
                                            foreach($starTrack_label_layout_options as $option){
                                                if(isset($general_settings['label_layout_type_starTrack'])){
                                                    if($option == $general_settings['label_layout_type_starTrack']){ ?>
                                                        <option value="<?php echo $option;?>" selected="selected"><?php _e( $option, 'wf-shipping-auspost');?></option>
                                            <?php   }else{ ?>
                                                        <option value="<?php echo $option;?>"><?php _e( $option, 'wf-shipping-auspost');?></option>
                                            <?php    }
                                                }
                                                else{ ?>
                                                    <option value="<?php echo $option;?>"><?php _e( $option, 'wf-shipping-auspost');?></option>
                                ...
                                <td><span class="woocommerce-help-tip" data-tip="pp - <?php _e('labels per page', 'wf-shipping-auspost')?>"></span></td>
                            </tr>
                <?php   }?>

                        <tr>
                            <td><?php _e('Parcel Post', 'wf-shipping-auspost')?></td>
                            <td>
                        ...
                        <tr>
                            <td><?php _e('Express Post', 'wf-shipping-auspost')?></td>
                            <td>
                        ...
                        <tr>
                            <td><?php _e('Australia Post International', 'wf-shipping-auspost')?></td>
                            <td>
                                <select name="wf_auspost_label_layout_international" id="wf_auspost_label_layout_international" style="width:200px;">
                                    <option value="A4-1pp"><?php _e('A4-1pp', 'wf-shipping-auspost')?></option>
                                </select>
                            </td>
                            <td><span class="woocommerce-help-tip" data-tip="pp - <?php _e('labels per page', 'wf-shipping-auspost')?>"></span></td>
                        </tr>
	...
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Weight/ Dimension Unit','wf-shipping-auspost') ?></label>
		</td>
		<td>
			<fieldset style="padding:3px;">
				<b>( KG/CM )</b><br/>
	...
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Shipment Content','wf-shipping-auspost') ?></label>
		</td>
		<td>
			<fieldset style="padding:3px;">
				<input class="input-text regular-input " type="text" name="wf_australia_post_ship_content" id="wf_australia_post_ship_content" style="" value="<?php echo (!empty($general_settings['ship_content'])) ? $general_settings['ship_content'] :''; ?>" placeholder='Shipment Contents'> <span class="woocommerce-help-tip" data-tip="<?php _e('Provide here a description about the shipment content.','wf-shipping-auspost') ?>" ></span>
			</fieldset>
		</td>
	</tr>
	...
	<tr valign="top" >
		<td style="width:50%;font-weight:800;">
			<label for="wf_australia_post_"><?php _e('Custom Shipment Message','wf-shipping-auspost') ?></label>
		</td>
		<td>
			<fieldset style="padding:3px;">
				<input class="input-text regular-input " type="text" name="wf_australia_post_custom_message" id="wf_australia_post_custom_message" style="width:800px;" value="<?php _e($shipment_custom_message, 'wf-shipping-auspost') ?>" placeholder="<?php _e('Your order was shipped on [DATE] via [SERVICE]. To track shipment, please follow the link of tracking ID(s) [ID]', 'wf-shipping-auspost')?>"> 		
			</fieldset>
			<br/>
		</td>
	</tr>
    <?php if($this->rate_type == 'startrack'): ?>
        <tr valign="top" >
            <td style="width:50%;font-weight:800;">
                <label for="wf_australia_post_"><?php _e('Dangerous Goods (StarTrack)','wf-shipping-auspost') ?></label>
            </td>
            <td>
                <fieldset style="padding:3px;">
                    <?php _e('Enable', 'wf-shipping-auspost');?>
                    <input class="input-text regular-input " type="checkbox" name="enable_dangerous_goods_configuration_startrack" id="enable_dangerous_goods_configuration_startrack" style="" value="yes" <?php echo (isset($general_settings['enable_dangerous_goods_configuration_startrack']) && $general_settings['enable_dangerous_goods_configuration_startrack'] ==='yes') ? 'checked' : ''; ?>><span class="woocommerce-help-tip" data-tip="<?php _e('Enable this option if you are shipping dangerous goods using StarTrack.','wf-shipping-auspost') ?>" ></span>
                </fieldset>
                <br/>
            </td>
        </tr>
        <tr valign="top" >
            <td style="width:50%;font-weight:800;" id="dangerous_goods_table_title_startrack_elex">
                <?php _e('Configure Dangerous Goods (StarTrack)','wf-shipping-auspost') ?>
            </td>
        </tr>
        <table style="width:50%;font-weight:800;" class="dangerous_goods_table_startrack_elex widefat">
            <thead>
                <tr>
                   <th><b><?php _e('UN number', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Technical Name', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Class Division', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Subsidiary Risk', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Packing Group Designator', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Outer Packaging Type', 'wf-shipping-auspost');?></b></th>
                   <th><b><?php _e('Outer Packaging Quantity', 'wf-shipping-auspost');?></b></th>
                   <th></th>
                </tr>
            </thead>
            <tfoot>
                <tr>
                    <th colspan="3">
                        <button id="insert_new_row_dangerous_goods" class="button button-secondary"><?php _e( 'Add', 'wf-shipping-auspost' ); ?></button>
                        <button id="remove_selected_dangerous_goods" class="button button-secondary"><?php _e( 'Remove selected UN numbers', 'wf-shipping-auspost' ); ?></button>
                    </th>
                </tr>
            </tfoot>
            <tbody>
              ...
              foreach($general_settings['dangerous_goods_descriptions'] as $dangerous_goods_descriptions_key => $dangerous_goods_descriptions){
                ?>
                <tr>
                    <th><input type="text" style="width: 95%; padding: 4px 7px;" name="dangerous_goods_un_number[<?php echo $count;?>]" value="<?php echo $dangerous_goods_descriptions_key;?>"></th>
                    <th><input type="text" style="padding: 4px 7px;" name="dangerous_goods_technical_name[<?php echo $count;?>]" value="<?php echo $dangerous_goods_descriptions['technical_name'];?>"></th>
                    <th>
                        <select name="dangerous_goods_class_division[<?php echo $count;?>]" id="dangerous_goods_class_division_startrack_auspost_elex">
                            <?php foreach($starTrack_dangerous_goods_class_division as $class_division_value => $class_division_description){
                                if($class_division_value == $dangerous_goods_descriptions['class_division']){
                                    echo "<option value='".$class_division_value."' selected='selected'>".$class_division_description."</option>";
                                }else{
                                    echo "<option value='".$class_division_value."'>".$class_division_description."</option>";
                                }
                            }
                            ?>
                        </select>
                    ...
                    <th>
                        <select name="dangerous_goods_subsidiary_risk[<?php echo $count;?>]" id="dangerous_goods_subsidiary_risk_startrack_auspost_elex">
                            <?php foreach($starTrack_dangerous_goods_class_division as $class_division_value => $class_division_description){
                    ...
                    <th>
                        <select name="dangerous_goods_packing_group_designator[<?php echo $count;?>]" id="dangerous_goods_packing_group_designator_startrack_auspost_elex">
                            <?php foreach($startrack_dangerous_goods_packing_group_designator as $packing_group_designator_value => $packing_group_designator_label){
                                if($packing_group_designator_value == $dangerous_goods_descriptions['packing_group_designator']){
                                    echo "<option value='".$packing_group_designator_value."' selected='selected'>".$packing_group_designator_label."</option>";
                                }else{
                                    echo "<option value='".$packing_group_designator_value."'>".$packing_group_designator_label."</option>";
                                }
                    ...
                    <th>
                        <select name="dangerous_goods_outer_packaging_type[<?php echo $count;?>]" id="dangerous_goods_outer_packaging_type_startrack_auspost_elex">
                            <?php foreach($startrack_packaging_types as $package_type => $package_type_label){
                                if($package_type == $dangerous_goods_descriptions['outer_packaging_type']){
                                    echo "<option value='".$package_type."' selected='selected'>".$package_type_label."</option>";
                                }else{
                                    echo "<option value='".$package_type."'>".$package_type_label."</option>";
                                }
                            } ?>
                        </select>
                    </th>
                    <th><input type="text" style="width: 40%; padding: 4px 7px;" name="dangerous_goods_outer_packaging_quantity[<?php echo $count;?>]" value="<?php echo $dangerous_goods_descriptions['outer_packaging_quantity'];?>"></th>
    	...
    	<tr>
    		<td colspan="2" style="text-align:center;">
    			<input type="submit" value="<?php _e('Save Changes','wf_australia_post') ?>" class="button button-primary" name="wf_australia_post_label_save">
    		</td>
    	</tr>
    </table>
