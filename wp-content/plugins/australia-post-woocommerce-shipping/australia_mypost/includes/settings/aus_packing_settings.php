...
if (isset($_POST['wf_aus_packing_save_changes_button'])) {
	...
	if ( 'weight' === $wc_main_settings['packing_method'] ) {
		...
		$wc_main_settings['weight_packing_process'] = sanitize_text_field($_POST['wf_mypost_shipping_weight_packing_process']);
		$weight_box                                 = array();
	...
	if ( 'box_packing' === $wc_main_settings['packing_method'] ) {
		...
		update_option('ausmypost_stored_pre_defined_boxes', $stored_pre_defined_boxes_auspost);
		update_option('ausmypost_stored_custom_boxes', $stored_custom_boxes_auspost);
	}
	...
	if ( 'weight' === $wc_main_settings['packing_method'] ) {
		...
		update_option( 'aus_mypost_stored_custom_boxes', $stored_custom_boxes_auspost );
	}
}
		...
		<tr id="weight_based_option">
			...
			foreach ($weight_type as $key => $value) {
				if ($key === $slected_weight_type) {
					echo '<input class="input-text regular-input " type="radio" name="wf_mypost_shipping_weight_packing_process" id="wf_mypost_shipping_weight_packing_process" style="" value="' . $key . '" checked="true" placeholder=""> ' . $value . ' ';
				} else {
					echo '<input class="input-text regular-input " type="radio" name="wf_mypost_shipping_weight_packing_process" id="wf_mypost_shipping_weight_packing_process" style="" value="' . $key . '"  placeholder=""> ' . $value . ' ';
				}
			}
			?>
			</fieldset>
