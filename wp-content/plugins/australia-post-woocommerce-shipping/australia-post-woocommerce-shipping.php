...
if (!defined('WF_AUSTRALIA_MYPOST_ID')) {
	define('WF_AUSTRALIA_MYPOST_ID', 'wf_australia_mypost');
}
if (!defined('WF_AUSTRALIA_POST_URL')) {
	define('WF_AUSTRALIA_POST_URL', plugin_dir_url(__FILE__));
}
if (!defined('WF_AUSTRALIA_POST_PATH')) {
	define('WF_AUSTRALIA_POST_PATH', plugin_dir_path(__FILE__));
}
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))) || ( is_multisite() && is_plugin_active_for_network('woocommerce/woocommerce.php') )) {
	...
	if (!class_exists('wf_au_post_woocommerce_shipping_setup')) {
		...
		class wf_au_post_woocommerce_shipping_setup {
			...
			public function __construct() {
				...
				include_once 'australia_post/includes/class-wf-australia-post-shipping-custom-checkout-fields.php';
			}

			public function init() {
				if (!class_exists('wf_order')) {
					include_once __DIR__ . '/australia_post/includes/class-wf-legacy.php';
				}
			...
			public function wf_init() {
				include_once __DIR__ . '/australia_post/includes/class-wf-tracking-admin.php';
				...
				if ($this->contracted_rates) {
					include_once 'australia_post/includes/class-wf-australia-post-shipping-admin.php';
				}
			...
			public function wf_plugin_action_links( $links) {
				$plugin_links = array(
					'<a href="' . admin_url('admin.php?page=' . wf_get_settings_url() . '&tab=shipping&section=wf_australia_post') . '">' . __('Australia Post( E-parcel & Startrack)', 'wf_australia_post') . '</a>',
					'<a href="' . admin_url('admin.php?page=' . wf_get_settings_url() . '&tab=shipping&section=wf_australia_mypost') . '">' . __('MyPost Business', 'wf_australia_post') . '</a>',
			...
			public function wf_australia_post_init() {
				include_once 'australia_post/includes/class-wf-australia-post-shipping.php';
				include_once 'australia_post/includes/class-elex-australia-post-functions.php';
			}
			...
			public function wf_australia_post_scripts() {
				...
				if ($page == 'auspost_manifest') {
					wp_enqueue_script('bootstrap_js', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/js/modal.min.js', basename(__FILE__)), array(), '2.0.0', true);
					wp_enqueue_style('model_css', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/css/modal.min.css'));
				}

				wp_enqueue_script('jquery-ui-sortable');
				wp_enqueue_script('jquery.validate.min', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/js/jquery.validate.min.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_enqueue_script('elex-auspost-custom', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/js/elex-auspost-custom.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_enqueue_script('wf_common', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/js/wf_common.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_localize_script('elex-auspost-custom', 'elex_auspost_custom', array('contracted_rates' => ''));
				wp_enqueue_style('font-awesome.min', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/css/font-awesome/css/font-awesome.css'));

				wp_enqueue_script('paging', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_post/js/paging.js'));
			}
	...
	if (!class_exists('wf_au_mypost_woocommerce_shipping_setup')) {

		class wf_au_mypost_woocommerce_shipping_setup {
		
			public function __construct() {
				$this->wf_init();
				add_action('init', array($this, 'init'));
				//add_filter('plugin_action_links_' . plugin_basename(__FILE__), array($this, 'wf_plugin_action_links'));
				add_action('woocommerce_shipping_init', array($this, 'wf_australia_mypost_init'));
				add_filter('woocommerce_shipping_methods', array($this, 'wf_australia_mypost_add_method'));
				add_filter('admin_enqueue_scripts', array($this, 'wf_australia_mypost_scripts'));
				include_once 'australia_mypost/includes/class-wf-australia-post-shipping-custom-checkout-fields.php';
			}

			public function init() {
				if (!class_exists('wf_order')) {
					include_once __DIR__ . '/australia_mypost/includes/class-wf-legacy.php';
				}
			...
			public function wf_init() {
				...
				$this->my_post_contracted_rates = isset($this->settings['client_account_name']) ? true : false;
				$mode_check                     = get_option('ELEX_AUS_POST_IS_MYPOST_BUSINESS_SANDBOX_MODE', 'off');
				
				if ( 'on' == $mode_check ) {
					$this->settings['mode_check'] = 'test';
					update_option('woocommerce_' . WF_AUSTRALIA_MYPOST_ID . '_settings', $this->settings);
				} else {
					$this->settings['mode_check'] = 'live';
					update_option('woocommerce_' . WF_AUSTRALIA_MYPOST_ID . '_settings', $this->settings);
				}

				if ($this->my_post_contracted_rates) {

					include_once 'australia_mypost/includes/class-wf-australia-mypost-shipping-admin.php';
				}
				if ( null == $this->settings) {
					include_once 'australia_mypost/includes/class-wf-australia-mypost-shipping-admin.php';
				}

				/** Mypost Bulk shipments,Orders,Pickup Generation */
				include_once 'australia_mypost/includes/html-wf-australia-mypost-bulk-shipments-order.php';
			...
			public function wf_australia_mypost_init() {
				include_once 'australia_mypost/includes/class-wf-australia-post-shipping.php';
				include_once 'australia_mypost/includes/class-elex-australia-post-functions.php';
			}

			public function wf_australia_mypost_add_method( $methods) {
				$methods[] = 'wf_australia_mypost_shipping';
				return $methods;
			}

			public function wf_australia_mypost_scripts() {


				$page = ( isset($_GET['page']) ? $_GET['page'] : '' );
				if ($page == 'elex_mypost_bulk_shipment_order') {
					wp_enqueue_script('bootstrap_js', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/modal.min.js', basename(__FILE__)), array(), '2.0.0', true);
					wp_enqueue_style('model_css', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/css/modal.min.css'));
					
					wp_register_style('datatables_style', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/css/datatables.min.css'));
					wp_enqueue_style('datatables_style');
					wp_register_script('datatables', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/datatables.min.js'), array('jquery'), true);
					wp_enqueue_script('datatables');
				}

				wp_enqueue_script('jquery-ui-sortable');
				wp_enqueue_script('jquery.validate.min', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/jquery.validate.min.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_enqueue_script('elex-auspost-custom', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/elex-auspost-custom.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_enqueue_script('wf_common', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/wf_common.js', basename(__FILE__)), array(), '2.0.0', true);
				wp_localize_script('elex-auspost-custom', 'elex_ausmypost_custom', array('contracted_rates' => '','ajax_url' => admin_url('admin-ajax.php?action=elex_bulk_orders_table_data'),));
				wp_enqueue_style('font-awesome.min', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/css/font-awesome/css/font-awesome.css'));

				wp_enqueue_script('paging', plugins_url(basename(plugin_dir_path(__FILE__)) . '/australia_mypost/js/paging.js'));
			}

		}
		new wf_au_mypost_woocommerce_shipping_setup();
	}
...
<?php
			...
			public function init() {
				include_once 'australia_post/includes/class-wf-tracking-admin.php';
				if (is_admin()) {
					$settings         = get_option('woocommerce_' . WF_AUSTRALIA_POST_ID . '_settings', null);
...
/* Australia Post (E-parcel & Startrack) Manifest Functions */
...
function auspost_bulk_order_manifest_menu_page() {
	include_once 'australia_post/includes/manifest_html.php';
}
