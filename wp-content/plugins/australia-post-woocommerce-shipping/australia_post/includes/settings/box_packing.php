			...
			.aus_services td.sort {
				...
				if (is_array($pack_type) && !empty($pack_type)) {
					foreach ($pack_type as $k => $v) {
						$selected       = ( 'YP' == $k )? 'selected="selected"' : '';
		...
		<table class="aus_boxes widefat">
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'Is Letter', 'wf-shipping-auspost' ); ?></th>
					<?php 
					}
					?>
					<?php if ( 'startrack' == $this->rate_type ) { ?>
						<th><?php esc_html_e( 'Packaging Type', 'wf-shipping-auspost' ); ?>
							<span class="woocommerce-help-tip" data-tip="<?php esc_html_e('This option is for StarTrack only. Please select NONE if you are using Australia Post.', 'wf-shipping-auspost'); ?>"></span>
						</th>
					<?php } else { ?>
						...
						foreach ($auspost_stored_pre_defined_boxes as $key => $box) { 
							...
							<tr>
								...
								if (!$this->contracted_rates) { 
									if ( 'PD' == $box['pack_type'] ) {
										?>
											<td><input type="checkbox" name="boxes_is_letter[<?php echo esc_attr( $count_pre_defined ); ?>]" <?php checked( !empty($box['is_letter'])? $box['is_letter']:'', true ); ?>/></td>
										<?php
									} else {
										?>
											<td><input type="checkbox" name="boxes_is_letter[<?php echo esc_attr( $count_pre_defined ); ?>]" <?php checked( !empty($box['is_letter'])? $box['is_letter']:'', true ); ?> /></td>
				...
				if ($this->contracted_rates) {
					if (!empty($boxes_to_display)) {
						foreach ($boxes_to_display as $key => $box) {
							if (( 'YP' == $box['pack_type'] ) || 'startrack' == $this->rate_type ) {
								$box_name = $box['name'];
								?>
								<tr>
									<td class="check-column"><input type="checkbox" /></td>
									...
									if (!$this->contracted_rates) { 
										if ( 'PD' == $box['pack_type'] ) {
											?>
													<td><input type="checkbox" class="box_settings_tab" name="boxes_is_letter[<?php echo esc_attr( $count_custom ); ?>]" <?php checked( !empty($box['is_letter'])?$box['is_letter']:'', true ); ?>/></td>
												<?php
										} else {
											?>
													<td><input type="checkbox" class="box_settings_tab" name="boxes_is_letter[<?php echo esc_attr( $count_custom ); ?>]" <?php checked( !empty($box['is_letter'])?$box['is_letter']:'', true ); ?> /></td>
									...
									<?php if ($this->contracted_rates) { ?>
										<td>
											<select class="box_settings_tab" name="boxes_pack_type[<?php echo esc_attr( $count_custom ); ?>]" >
											<?php
											if ( 'startrack' == $this->rate_type ) { 
			...
			jQuery(window).load(function(){
				...
				jQuery('.aus_boxes .insert').click( function() {
					...
					var code = '<tr class="new">\
									...
									<?php } elseif ( 'startrack' == $this->rate_type ) { ?>
										<td><select name="boxes_pack_type[' + size + ']" >\
										<option value="NONE" ><?php esc_html_e('NONE', 'wf-shipping-auspost'); ?></option>\
										<option value="CTN" ><?php esc_html_e('Carton', 'wf-shipping-auspost'); ?></option>\
										<option value="PAL" ><?php esc_html_e('Pallet', 'wf-shipping-auspost'); ?></option>\
										<option value="SAT" ><?php esc_html_e('Satchel', 'wf-shipping-auspost'); ?></option>\
										<option value="BAG" ><?php esc_html_e('Bag', 'wf-shipping-auspost'); ?></option>\
										<option value="ENV" ><?php esc_html_e('Envelope', 'wf-shipping-auspost'); ?></option>\
										<option value="ITM" ><?php esc_html_e('Item', 'wf-shipping-auspost'); ?></option>\
										<option value="JIF" ><?php esc_html_e('Jiffy Bag', 'wf-shipping-auspost'); ?></option>\
										<option value="SKI" ><?php esc_html_e('Skid (up to 500kg)', 'wf-shipping-auspost'); ?></option>\
										</select></td>\
									<?php } else { ?>
