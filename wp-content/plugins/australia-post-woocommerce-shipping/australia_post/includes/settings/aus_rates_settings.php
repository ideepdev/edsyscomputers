...
if (isset($_POST['wf_aus_rates_save_changes_button'])) {
	...
	$wc_main_settings['title']                = ( isset($_POST['wf_australia_post_title']) ) ? sanitize_text_field($_POST['wf_australia_post_title']) : ( ( isset($this->settings['wf_australia_post_starTrack_rates']) && ( 'yes' == $this->settings['wf_australia_post_starTrack_rates'] ) ) ? __('StarTrack', 'wf-shipping-auspost') :   __('Australia Post', 'wf-shipping-auspost') );
	...
	$wc_main_settings['combined_export_tool_show_rate_separate']  = ( isset($_POST['elex_australia_post_combined_export_tool_show_rate_separate']) && 'yes' == $wc_main_settings['combined_export_tool_enable'] ) ? 'yes' : '';
...
} elseif (empty($this->settings['wf_australia_post_starTrack_api_account_no'])) {
	update_option('insufficient_authentication_data', 'yes');
}

if (!empty($auspost_customer_account_error_on_rates_settings)) {
	echo '<div class="error"><p>' . esc_html($auspost_customer_account_error_on_rates_settings, 'wf-shipping-auspost') . '</p></div>';
...
<table style="width:100%;table-layout:fixed; ">
	...
	<tr valign="top">
		<td style="width:30%;font-weight:800;">
			<label for="wf_australia_post_delivery_time_title"><?php esc_html_e('Estimated Delivery Date Settings', 'wf-shipping-auspost'); ?></label>
		</td>
		<td scope="row" class="titledesc" style="display: block;margin-bottom: 20px;margin-top: 3px;">            
			<fieldset style="padding:3px;">
				<input class="input-text regular-input" type="checkbox" name="wf_australia_post_estimated_delivery_date_enabled" id="wf_australia_post_estimated_delivery_date_enabled" value="yes" <?php echo ( isset($general_settings['aus_post_estimated_delivery_date_enabled']) && 'yes' === $general_settings['aus_post_estimated_delivery_date_enabled'] ) ? 'checked' : ''; ?> placeholder=""> <?php esc_html_e('Enable', 'wf-shipping-auspost'); ?> 
				<span class="woocommerce-help-tip" data-tip="<?php esc_attr_e('On enabling this, the estimated delivery date will be shown in the cart/checkout page.', 'wf-shipping-auspost'); ?>"></span>
			</fieldset>
			...
			<fieldset style="padding:3px;" class="australila_post_estimated_delivery">
						...
						<?php esc_html_e('Working Days', 'wf-shipping-auspost'); ?>
						<span class="woocommerce-help-tip australila_post_estimated_delivery" data-tip="<?php esc_attr_e('Configure the regular working days. The estimated delivery date will be calculated based on this. The shipment is supposed to happen only on working days.', 'wf-shipping-auspost'); ?>"></span></br>
						<select class="chosen_select"  style="width: 50%;"  name="wf_australia_post_working_days[]" multiple="multiple">
							<option value="Sun" <?php echo ( in_array('Sun', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Sunday', 'wf-shipping-auspost'); ?></option>
							<option value="Mon" <?php echo ( in_array('Mon', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Monday', 'wf-shipping-auspost'); ?></option>
							<option value="Tue" <?php echo ( in_array('Tue', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Tuesday', 'wf-shipping-auspost'); ?></option>
							<option value="Wed" <?php echo ( in_array('Wed', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Wednesday', 'wf-shipping-auspost'); ?></option>
							<option value="Thu" <?php echo ( in_array('Thu', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Thursday', 'wf-shipping-auspost'); ?></option>
							<option value="Fri" <?php echo ( in_array('Fri', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Friday', 'wf-shipping-auspost'); ?></option>
							<option value="Sat" <?php echo ( in_array('Sat', $aus_post_working_days) )? 'selected':''; ?> ><?php esc_html_e('Saturday', 'wf-shipping-auspost'); ?></option> 
						</select>
			</fieldset>            
			<label for="wf_australia_cut_off_time_lable" class="australila_post_estimated_delivery">
			<fieldset style="padding:3px;" class="australila_post_estimated_delivery">
				<?php esc_html_e('Cut-off Time', 'wf-shipping-auspost'); ?> <span class="woocommerce-help-tip australila_post_estimated_delivery" data-tip="<?php esc_attr_e('Configure the cut-off time for your shipment. The orders placed after the cut-off time will be shipped on the next working day. The estimated delivery date will be displayed based on this.', 'wf-shipping-auspost'); ?>"></span> </br>
				<input class="rates_tab_field" type="time" name="wf_australia_post_cut_off_time" id="wf_australia_post_cut_off_time"  value="<?php echo ( isset($general_settings['aus_post_cut_off_time']) ) ? esc_attr( $general_settings['aus_post_cut_off_time'] ) : ''; ?>" placeholder="Cut-off_time"> 
			</fieldset>
			<fieldset style="padding:3px;" class="australila_post_estimated_delivery">
				<?php esc_html_e('Lead Time', 'wf-shipping-auspost'); ?> <span class="woocommerce-help-tip australila_post_estimated_delivery" data-tip="<?php esc_attr_e('Add the number of days before you can initiate the delivery process.', 'wf-shipping-auspost'); ?>"></span> </br>
				<input class="" type="number" name="wf_australia_post_estimated_delivery_lead_time" id="wf_australia_post_estimated_delivery_lead_time"  value="<?php echo ( isset($general_settings['aus_post_estimated_delivery_lead_time']) ) ? esc_attr( $general_settings['aus_post_estimated_delivery_lead_time'] ) : ''; ?>" style="width:70px;" > 
			</fieldset>
			<fieldset style="padding:3px;" class="australila_post_estimated_delivery">
				<?php esc_html_e('View Option', 'wf-shipping-auspost'); ?> <span class="woocommerce-help-tip australila_post_estimated_delivery" data-tip="<?php esc_attr_e("Choose the format to display estimated delivery date - 'Days' or 'Date'", 'wf-shipping-auspost'); ?>"></span> </br>
				<div style="padding-top:5px ;">
				<input class="" type="radio" name="wf_australia_post_estimated_delivery_view_option" id="wf_australia_post_estimated_delivery_view_option_days"  value="days" <?php echo ( ( isset($general_settings['aus_post_estimated_delivery_view_option']) && 'days' == $general_settings['aus_post_estimated_delivery_view_option'] ) || ( !isset($general_settings['aus_post_estimated_delivery_view_option']) ) ) ? 'checked' : ''; ?> >
				<label for="Days" style="padding-right:5px ;">Days</label>
				<input class="" type="radio" name="wf_australia_post_estimated_delivery_view_option" id="wf_australia_post_estimated_delivery_view_option_date"  value="date" <?php echo ( isset($general_settings['aus_post_estimated_delivery_view_option']) && 'date' == $general_settings['aus_post_estimated_delivery_view_option'] ) ? 'checked' : ''; ?> >  
		...
		<td scope="row" class="titledesc" style="display: block;margin-bottom: 20px;margin-top: 3px;">
			...
			if (!$this->contracted_rates) {
				?>
						...
						<fieldset style="padding:3px;">
							<input class="input-text regular-input " type="checkbox" name="wf_australia_post_show_authority_to_leave_checkout_field" id="wf_australia_post_show_authority_to_leave_checkout_field" style="" value="yes" <?php echo ( isset($general_settings['show_authority_to_leave_checkout_field']) && 'yes' === $general_settings['show_authority_to_leave_checkout_field'] ) ? 'checked' : ''; ?> placeholder=""> <?php esc_html_e('Authority To Leave', 'wf-shipping-auspost'); ?> <span class="woocommerce-help-tip" data-tip="<?php esc_attr_e('Enable this field to let customers choose whether they want to apply Authority To Leave option', 'wf-shipping-auspost'); ?>"></span>
						</fieldset>
						<fieldset style="padding:3px;">
	...
	<tr valign="top">
		<td style="width:30%;font-weight:800;">
			<label for="elex_australia_post_combined_export_tool"><?php esc_html_e('Combined Export Tool', 'wf-shipping-auspost'); ?></label>
		</td>
		<td scope="row" class="titledesc" style="display: block;margin-bottom: 20px;margin-top: 3px;">
			<fieldset style="padding:3px;">
				<input class="input-text regular-input " type="checkbox" name="elex_australia_post_combined_export_tool_enable" id="elex_australia_post_combined_export_tool_enable" style="" value="yes" <?php echo ( isset($general_settings['combined_export_tool_enable']) && 'yes' === $general_settings['combined_export_tool_enable'] ) ? 'checked' : ''; ?> > <?php esc_html_e('Enable', 'wf-shipping-auspost'); ?> <span class="woocommerce-help-tip" data-tip="<?php esc_attr_e('Enable AusPost Combined Export tool that retrieves estimated duties and taxes for the shipment.', 'wf-shipping-auspost'); ?>"></span>
			</fieldset>
		</td>
