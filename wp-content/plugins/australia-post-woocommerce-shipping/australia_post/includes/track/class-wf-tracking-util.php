...
class WfTrackingUtil {
	...
	/**
	 * The default_tracking_data can be obtained by calling load_tracking_data by setting force_default param true.
	 */
	...
	public static function get_default_shipment_message_placeholder() {
		$message = 'Your order was shipped on [DATE] via [SERVICE]. To track shipment, please follow the link of tracking ID(s) [ID]';
		return $message;
	}

	public static function get_shipment_custom_message() {
		$auspost_settings = get_option( 'woocommerce_' . WF_AUSTRALIA_POST_ID . '_settings' );
		return $auspost_settings['custom_message'];
	}
	...
	public static function get_shipment_display_default_message( $shipment_result_array ) {
		...
		if (isset($general_settings['wf_australia_post_starTrack_rates_selected']) && ( true == $general_settings['wf_australia_post_starTrack_rates_selected'] )) {
			$shipment_result_array['message'] = str_replace('Australia Post', 'StarTrack (Australia Post)', $shipment_result_array['message']);
		}

		if ( isset( $shipment_result_array['tracking_info'] ) ) {
			$message      .= $shipment_result_array['message'];
			$sub_message_1 = ' To track shipment, please follow the tracking ID(s)';
			$sub_message_2 = '';
	...
	public static function update_tracking_data ( $order_id, $shipment_id_cs, $shipping_service, $shipment_source_key, $shipment_result_key, $order_date = '' ) {
