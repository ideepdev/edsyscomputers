...
class wf_australia_post_shipping extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_title       = __('Australia Post( E-parcel & Startrack)', 'wf-shipping-auspost');
		...
		if ($this->contracted_rates) {
			...
			if (is_array($package_requests)) {
				...
				if ($this->group_shipping_enabled == false || $this->is_international) {
					...
					$request_params_startrack               				 = new stdClass();
					$request_params_eparcel               				 	 = new stdClass();
					$extra_cover_package_price 								 = array();
					foreach ($package_requests as $key => $package_request) {
						...
						if ( $this->is_international && $this->is_elex_combined_export_tool_enable && $this->is_elex_combined_export_tool_enable == 'yes' ) {
							if (is_array($package_request) && !empty($package_request) && isset($package_request['packed_products']) && is_array($package_request['packed_products']) && !empty($package_request['packed_products']) ) {
								$estimated_duties_and_taxes_for_the_shipment_request_body[$count_package_requests] = $this->estimated_duties_and_taxes_for_the_shipment_request_body($package_request['packed_products'] , $package['destination']);
							}
							
						}
						$extra_cover_package_price[$count_package_requests] = $package_request['InsuredValue']['Amount'] ? $package_request['InsuredValue']['Amount'] : 0;
						...
						if ($this->rate_type == 'startrack' && !$this->is_international) {
							...
							$request_params_startrack->shipments[] = $shipments;
						...
						if (!empty($this->api_pwd_auspost) && !empty($this->api_account_no_auspost)) {
							...
							*/
							...
							$shipments->to                       = $to;
							$shipments->items                    = array($items_node);
							$request_params_eparcel->shipments[] = $shipments;

						}
					}
					$startrack_service_rate_iteration = 0;
					...
					foreach ($this->general_settings['startrack_services'] as $settings_service_key => $settings_service_value) {
						$startrack_service_rate_iteration++;
						if ($request_params_startrack !== new stdClass()) {
							$shipments_data = $request_params_startrack;
							if ($settings_service_value['enabled']) {
								foreach ($shipments_data->shipments as $key => $value) {
									$request_params_startrack->shipments[$key]->items[0]['product_id'] = $settings_service_key;
								...
								if (isset($vendor_elex_australia_post_startrack_account_number) && isset($vendor_elex_australia_post_startrack_api_password)) {
									$headers = $this->buildHttpHeaders($request_params_startrack, $vendor_elex_australia_post_startrack_account_number, $vendor_elex_australia_post_startrack_api_password, $vendor_elex_australia_post_api_key);
								} else {
									$headers = $this->buildHttpHeaders($request_params_startrack, $this->api_account_no_starTrack, $this->api_pwd_starTrack, $this->api_key_starTrack);
								}
								...
								$this->debug('StarTrack (AusPost) Contracted rate REQUEST: <pre>' . print_r($request_params_startrack, true) . '</pre>');
	
								$startrack_service_rates_for_package = $this->get_contracted_rates($endpoint, $request_params_startrack, $headers, 'startrack');
								...
								if (isset($startrack_service_rates_for_package['error_message'])) {
									...
									$startrack_service_rates = array_merge($startrack_service_rates, $startrack_service_rates_for_package);
								}
					...
					$eparcel_service_rate_iteration = 0;
					...
					foreach ($this->general_settings['services'] as $settings_service_key => $settings_service_value) {
						$shipments_data = $request_params_eparcel;
						$eparcel_service_rate_iteration++;
						$flag                    = false;
						...
						if ($request_params_eparcel !== new stdClass() ) {
							$shipments_data = $request_params_eparcel;
							if ($settings_service_value['enabled'] && $flag) {
								foreach ($shipments_data->shipments as $key => $value) {
									$request_params_eparcel->shipments[$key]->items[0]['product_id'] = $settings_service_key;
								...
								if ($this->vedor_api_key_enable && isset($vendor_elex_australia_post_account_number) && isset($vendor_elex_australia_post_api_password)) {
									$headers = $this->buildHttpHeaders($request_params_eparcel, $vendor_elex_australia_post_account_number, $vendor_elex_australia_post_api_password, $vendor_elex_australia_post_api_key);
								} else {
									$headers = $this->buildHttpHeaders($request_params_eparcel, $this->api_account_no_auspost, $this->api_pwd_auspost);
								}
	
								if ($this->settings['contracted_api_mode'] == 'live') {
									$endpoint = str_replace('test/', '', $endpoint);
								}
								$this->debug('AusPost Contracted rate REQUEST: <pre>' . print_r($request_params_eparcel, true) . '</pre>');
								$auspost_service_rates_for_package = $this->get_contracted_rates($endpoint, $request_params_eparcel, $headers, 'auspost');
								...
								if (isset($auspost_service_rates_for_package['error_message'])) {
									...
									$auspost_service_rates = array_merge($auspost_service_rates, $auspost_service_rates_for_package);
							...
							foreach ($startrack_service_rates as $rate) {
								...
								if (isset($rate['shipment_summary']) && !empty($rate['shipment_summary'])) {
									...
									if (isset($rates_price->total_cost)) {
										if ( $this->is_international && $this->is_elex_combined_export_tool_enable && $this->is_elex_combined_export_tool_enable == 'yes' && isset($estimated_duties_and_taxes_for_the_shipment_request_body) && $rate_include_gst > 0 ) {
											$estimated_duties_and_taxes_for_the_shipment_request_body[$count]->shipping_value = $rate_include_gst;
											...
											if ($estimated_duties_and_taxes_for_the_shipment_value > 0) {
												if (array_key_exists($rates_items->product_id, $estimated_duties_and_taxes_for_the_shipment_value_array)) {
													$estimated_duties_and_taxes_for_the_shipment_value_array[$rates_items->product_id] += $estimated_duties_and_taxes_for_the_shipment_value;
												} else {
													$estimated_duties_and_taxes_for_the_shipment_value_array[$rates_items->product_id] = $estimated_duties_and_taxes_for_the_shipment_value;
												}
												$rate_include_gst += $estimated_duties_and_taxes_for_the_shipment_value;
											}
										}
										...
										if (is_shop() || is_cart()) {
											...
											if (isset($settings_services[$rates_items->product_id]['extra_cover']) && ( $settings_services[$rates_items->product_id]['extra_cover'] == true )) {
												$add_extra_cover = true;
											}
										} elseif (isset($this->settings['show_insurance_checkout_field']) && ( ( $this->settings['show_insurance_checkout_field'] === 'yes' && $this->insurance_requested_at_checkout ) || ( $this->settings['show_insurance_checkout_field'] == '' && isset($settings_services[$rates_items->product_id]['extra_cover']) && $settings_services[$rates_items->product_id]['extra_cover'] == true ) )) {
											$add_extra_cover = true;
										}
										if ($add_extra_cover) {
											if ($this->is_international) {
												if (( $extra_cover_package_price[$count] != 0 ) && ( $extra_cover_package_price[$count] <= 100 )) {
													$rate_include_gst += 0; // extra cover fee for less than 100
												} elseif (( $extra_cover_package_price[$count] > 100 ) && ( $extra_cover_package_price[$count] <= 5000 )) {
													//$rate['cost'] += $this->extra_cover_cost['international'] * $extra_cover_package / 100; // extra cover fee for greater than 100
													$extra_cover_cal = $extra_cover_package_price[$count] / 100;
													if ( $extra_cover_package_price[$count] % 100 === 0 ) {
	
														$extra_cover_cal_multiplier = $extra_cover_cal - 1;
														$rate_include_gst          += $this->extra_cover_cost['international']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
												...
												if (( $extra_cover_package_price[$count] != 0 ) && ( $extra_cover_package_price[$count] <= 100 )) {
													...
													if ( $extra_cover_package_price[$count] % 100 === 0 ) {
														...
														$extra_cover_cal_multiplier = $extra_cover_cal - 1;
														$rate_include_gst          += $this->extra_cover_cost['domestic']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
										...
										if (isset($this->found_rates[$rates_items->product_id])) {
											$rate_include_gst = $this->found_rates[$rates_items->product_id]['cost'] + $rate_include_gst;
										}

										$this->prepare_rate($rates_items->product_id, $rates_items->product_id, $items_product_type, $rate_include_gst, $request_params_startrack);
									}
								}
								if ($count == ( $count_package_requests- 1 )) {
									$count = 0;
								} else {
									$count++;
								}
									...
									if (isset($rates_price->total_cost)) {
										...
										if ($add_extra_cover) {
											if ($this->is_international) {
												...
												if (( $extra_cover_package_price[$count] != 0 ) && ( $extra_cover_package_price[$count] <= 100 )) {
													$rate_include_gst += 0; // extra cover fee for less than 100
												} elseif (( $extra_cover_package_price[$count] > 100 ) && ( $extra_cover_package[$count] <= 5000 )) {
													//$rate['cost'] += $this->extra_cover_cost['domestic'] * $extra_cover_package / 100; // extra cover fee for greater than 100
													$extra_cover_cal = $extra_cover_package_price[$count] / 100;
													if ( $extra_cover_package_price[$count] % 100 === 0 ) {
										...
										$this->prepare_rate($rates_items->product_id, $rates_items->product_id, $items_product_type, $rate_include_gst, $request_params_eparcel);
									}
								}
								if ($count == ( $count_package_requests- 1 )) {
									$count = 0;
					...
					} elseif (empty($startrack_service_rates)) {
											...
											if ($settings_service['enabled'] == true) {
												...
												if ($postage_product['product_id'] === $key) {
													...
													if ( $this->is_international && $this->is_elex_combined_export_tool_enable && $this->is_elex_combined_export_tool_enable == 'yes' && isset($estimated_duties_and_taxes_for_the_shipment_request_body) && $rate['cost'] > 0 ) {
														if ($estimated_duties_and_taxes_for_the_shipment_value_array[$key] > 0) {
				...
				} elseif ($this->group_shipping_enabled == true && !$this->is_international) {
										...
										foreach ($settings_services as $key => $settings_service) {
											if ($settings_service['enabled'] == true) {
												...
												if ($postage_product['product_id'] === $key) {
													...
													} elseif (isset($this->settings['show_insurance_checkout_field']) && ( ( $this->settings['show_insurance_checkout_field'] === 'yes' && $this->insurance_requested_at_checkout ) || ( $this->settings['show_insurance_checkout_field'] == '' && isset($settings_service['extra_cover']) && $settings_service['extra_cover'] == true ) )) {
														$add_extra_cover = true;
													}
													if ($add_extra_cover) {
														if ($this->is_international) {
															if (( $extra_cover_package != 0 ) && ( $extra_cover_package <= 100 )) {
																$rate['cost'] += 0; // extra cover fee for less than 100
																...
																if ( $extra_cover_package % 100 === 0 ) {
																	...
																	$extra_cover_cal_multiplier = $extra_cover_cal - 1;
																	$rate['cost']              += $this->extra_cover_cost['international']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
															...
															if (( $extra_cover_package != 0 ) && ( $extra_cover_package <= 100 )) {
																...
																if ( $extra_cover_package % 100 === 0 ) {
																	...
																	$extra_cover_cal_multiplier = $extra_cover_cal - 1;
																	$rate['cost']              += $this->extra_cover_cost['domestic']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
			...
			if ($package_requests) {
				foreach ($package_requests as $key => $package_request) {
					...
					} elseif ($is_satchel_services_enabled && $package_weight > 5) {
	...
	private function validate_for_non_contracted_services( $validation_parameters) {
		...
		foreach ($validation_parameters['rates_response']->services->service as $quote) {
			...
			if (in_array($quote->code, $alternate_services_names)) {
				...
				} elseif ($validation_parameters['satchel_rate_status'] && $this->is_satchel($quote->code)) {
			...
			} elseif ($validation_parameters['service_code'] == $quote->code) {
				if (isset($this->custom_services[$rate_code]['delivery_confirmation']) && !empty($this->custom_services[$rate_code]['delivery_confirmation'])) {
					$delivery_confirmation = true;
				}
				$rate_cost = $quote->price;
	...
	private function get_shipping_rate_cost( $shipping_rate_cost_parameters) {
		...
		//Obtain the main service to which a sub service belongs to
		$main_service = ( is_array($shipping_rate_cost_parameters) && isset($shipping_rate_cost_parameters['auspost_sub_services']) &&isset($shipping_rate_cost_parameters['auspost_sub_services'][$rate_code]) && isset($shipping_rate_cost_parameters['auspost_sub_services'][$rate_code]['main_service']) ) ? $shipping_rate_cost_parameters['auspost_sub_services'][$rate_code]['main_service'] : '';
		...
		if (isset($shipping_rate_cost_parameters['auspost_services'][$rate_code]) || isset($shipping_rate_cost_parameters['auspost_sub_services'][$rate_code])) {
			...
			} elseif (!empty($main_service) && isset($shipping_rate_cost_parameters['auspost_services'][$main_service]) && $shipping_rate_cost_parameters['auspost_services'][$main_service]['adjustment']) {
			...
			} elseif (!empty($main_service) && isset($shipping_rate_cost_parameters['auspost_services'][$main_service]) && $shipping_rate_cost_parameters['auspost_services'][$main_service]['adjustment_percent']) {
			...
			if (is_shop() || is_cart()) {
				...
				} elseif (( isset($shipping_rate_cost_parameters['auspost_services'][$rate_code]) && $shipping_rate_cost_parameters['auspost_services'][$rate_code]['extra_cover'] == true ) || ( !empty($main_service) && isset($shipping_rate_cost_parameters['auspost_services'][$main_service]) && $shipping_rate_cost_parameters['auspost_services'][$main_service]['extra_cover'] == true )) {
					$add_extra_cover = true;
				}
			}
			if ($add_extra_cover == true) {
				if ($this->is_international) {
					if (( $shipping_rate_cost_parameters['extra_cover'] != 0 ) && ( $shipping_rate_cost_parameters['extra_cover'] <= 100 )) {
						$rate_cost += 0; // extra cover fee for less than 100
						...
						$extra_cover_cal = $shipping_rate_cost_parameters['extra_cover'] / 100;
						if ( $shipping_rate_cost_parameters['extra_cover'] % 100 === 0 ) {

							$extra_cover_cal_multiplier = $extra_cover_cal - 1;
							$rate_cost                 += $this->extra_cover_cost['international']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
					...
					if (( $shipping_rate_cost_parameters['extra_cover'] != 0 ) && ( $shipping_rate_cost_parameters['extra_cover'] <= 100 )) {
						...
						if ( $shipping_rate_cost_parameters['extra_cover'] % 100 === 0 ) {
							...
							$extra_cover_cal_multiplier = $extra_cover_cal - 1;
							$rate_cost                 += $this->extra_cover_cost['domestic']*$extra_cover_cal_multiplier; // extra cover fee for greater than 100
			...
			if (is_shop() || is_cart()) {
				if (isset($shipping_rate_cost_parameters['auspost_services'][$rate_code]) && $shipping_rate_cost_parameters['auspost_services'][$rate_code]['delivery_confirmation'] == true) {
					$add_signature = true;
				} elseif (!empty($main_service) && isset($shipping_rate_cost_parameters['auspost_services'][$main_service]['delivery_confirmation']) && ( $shipping_rate_cost_parameters['auspost_services'][$main_service]['delivery_confirmation'] == true )) {
					$add_signature = true;
				}
			} elseif (isset($this->settings['show_signature_required_field']) && $this->settings['show_signature_required_field'] == 'yes') {
				if ($this->signature_requested_at_checkout) {
					$add_signature = true;
				}
			} elseif (( ( isset($shipping_rate_cost_parameters['auspost_services'][$rate_code]) && in_array($rate_code, $this->delivery_confirmation) ) || ( !empty($main_service) && in_array($main_service, $this->delivery_confirmation) ) )) {
			...
			if (!$this->is_international) {
				...
				} elseif (strpos($rate_code, '_EXPRESS_SATCHEL_')) {
	...
	private function prepare_rate( $rate_code, $rate_id, $rate_name, $rate_cost, $package_request = '') {
		...
		} elseif (!empty($this->custom_sub_services[$rate_code])) {
		...
		} elseif (!empty($this->general_settings['sub_services'][$rate_code]['name'])) {
	...
	private function weight_only_shipping( $package) {
		...
		foreach ($pack as $package) {
			...
			sort($dimensions);
			$cubic_volume    = ( wc_get_dimension($dimensions[0], 'm', $this->dimension_unit) ) * ( wc_get_dimension($dimensions[1], 'm', $this->dimension_unit) ) * ( wc_get_dimension($dimensions[2], 'm', $this->dimension_unit) );
			$insurance_array = array(
			...
			$group = array(
				...
				'Dimensions' => array(
					'Length' => max(0.1, round($dimensions[2])),
					'Width' => max(0.1, round($dimensions[1])),
					'Height' => max(0.1, round($dimensions[0])),
	...
	private function box_shipping( $package) {
		...
		// Pack it
		$boxpack->pack();
		do_action( 'elex_aus_post_after_boxshipping_packing', $boxpack);
		...
		if ($rate_type == 'startrack') {
			...
			if (!empty($response_array)) {
				...
				if (isset($response_array->shipments)) {
					foreach ($response_array->shipments as $val) {
						$shipment_rates_result[]['shipment_summary'] = array(
							'prices'    => $val->shipment_summary,
							'items'     => $val->items,
							'options'   => $val->options
						);
					}
