...
class wf_australia_post_shipping_admin {
	...
	private function wf_user_permission() {
		...
		if ($current_user instanceof WP_User) {
				if (in_array('stores', $current_user->roles) || 
						...
						in_array('super-user', $current_user->roles)
					) {
						$user_ok = true;
				}
		}
		...
		if ($this->vendor_check) {
			...
			foreach ($order_items_data as $order_item_data) {
				...
				if (is_array($pack) && !empty($pack)) {
					foreach ($pack as $packable_item => $v) {
						if ($v->unpacked === true) {
							update_post_meta($order_id, 'wf_woo_australiapost_shipmentErrorMessage', 'Products Does not fit in the Weight Box');
							return array();
						}
						$all_items[] =   $v->packed;
					}
				...
				if (is_array($pack) && !empty($pack)) {
					foreach ($pack as $parcel) {
						if ($parcel->unpacked === true) {
							$this->debug('Item not packed in any box');
						} else {
							$this->debug('Packed ' . $parcel->name);
						}
						...
						if (empty($failed_shipment_order_packages) || ( !empty($failed_shipment_order_packages) && in_array($parcel_count, $failed_shipment_order_packages) )) {
							...
							$package_items          = array();
							$packed_products        =   isset($parcel->packed) ? $parcel->packed : $all_items;
							...
							foreach ($packed_products as $i => $packed_product) {
								...
								$weight_packed_products[$i]['name']     =  ( isset($packed_product->product_data['data']['name']) && !empty($packed_product->product_data['data']['name']) ) ? $packed_product->product_data['data']['name'] : '';
							}
							...
							if (!empty($packed_products) && is_array($packed_products)) {
								...
								foreach ($packed_products as $packed_product) {
									...
									$product = wc_get_product( isset( $packed_product->product_data['data']['variation_id'] ) ? $packed_product->product_data['data']['variation_id'] : $packed_product->product_data['data']['id']);
									...
									if (isset( $packed_product->product_data['data']['variation_id'] )) {
									...
									} elseif (isset( $packed_product->product_data['data']['id'] )) {
										$product_data      = $product->get_data();
										$product_dimension = array(
											$product_data['length'],
											$product_data['width'],
							...
							$cubic_volume =  ( ( wc_get_dimension(!empty($this->lengths_in_request_array[$parcel_count]) ? $this->lengths_in_request_array[$parcel_count] : round($parcel->length, 2), 'm', $this->dimension_unit) ) * ( wc_get_dimension(!empty($this->widths_in_request_array[$parcel_count]) ? $this->widths_in_request_array[$parcel_count] : round($parcel->width, 2), 'm', $this->dimension_unit) ) * ( wc_get_dimension(!empty($this->heights_in_request_array[$parcel_count]) ? $this->heights_in_request_array[$parcel_count] : round($parcel->height, 2), 'm', $this->dimension_unit) ) );
							...
							$group = array(
								'Name' =>  $parcel->name,
								'Weight' => array(
									'Value' => !empty($this->weights_in_request_array[$parcel_count]) ? $this->weights_in_request_array[$parcel_count] : round($parcel->weight, 3),
									'Units' => $this->weight_unit
								),
								'Dimensions' => array(
									'Length'    => !empty($this->lengths_in_request_array[$parcel_count]) ? $this->lengths_in_request_array[$parcel_count] : round($parcel->length, 2),
									'Width'     => !empty($this->widths_in_request_array[$parcel_count]) ? $this->widths_in_request_array[$parcel_count] : round($parcel->width, 2),
									'Height'    => !empty($this->heights_in_request_array[$parcel_count]) ? $this->heights_in_request_array[$parcel_count] : round($parcel->height, 2),
			...
			foreach ($orderItems as $item_id => $item) {
				...
				if (empty($failed_shipment_order_packages) || ( !empty($failed_shipment_order_packages) && in_array($parcel_count, $failed_shipment_order_packages) )) {
					...
					if ($item_data['variation_id']) {
						...
						// The girth should lie between 16cm and 140cm for international shipments
						if ($domestic == 'no' && ( $girth < 16 || $girth > 140 )) {
							update_post_meta($order_id, 'wf_woo_australiapost_shipmentErrorMessage', __('<b>Error</b>: Girth of the product %s should lie in between 16cm and 140cm.', 'wf-shipping-auspost'));
							return;
						}
					}
	...
	private function get_selected_shipping_service_id( $postage_products_type_and_product_ids, $serviceName, $order) {
		...
		foreach ($postage_products_type_and_product_ids as $postage_products_type_and_product_id_key => $postage_products_type_and_product_id_value) {
			...
			} elseif ($serviceName == $postage_products_type_and_product_id_value) {
				$postage_service_code = $postage_products_type_and_product_id_value;
			}
		}
	...
	public function wf_create_shipment( $order) {
		...
		} elseif ($this->is_request_bulk_startrack_shipment) {
			if ($this->rate_type == 'startrack') {
								...
								foreach ($auspost_services as $auspost_service) {
									if ($auspost_service['product_id'] == $create_shipment_service) {
										if ($auspost_service['group'] == 'Express Post') {
											...
											break;
										} elseif ($auspost_service['group'] == 'Parcel Post') {
						...
						if ($selected_service_type == 'StarTrack' && isset($package_request['cubic_volume']) && $package_request['cubic_volume'] > 0 && $this->packing_method == 'weight') {
							...
							if ( is_array($postage_products_service_options)  && !empty($postage_products_service_options)  && isset($postage_products_service_options[$create_shipment_service]) && isset($postage_products_service_options[$create_shipment_service]['options']) && is_array($postage_products_service_options[$create_shipment_service]['options']) &&  !empty($postage_products_service_options[$create_shipment_service]['options']) ) {
								...
								if ($shipement_service_options['authority_to_leave_option']) {
									...
									} elseif ( is_array($available_shipping_services) && !empty($available_shipping_services) && isset($available_shipping_services[$create_shipment_service]) && is_array($available_shipping_services[$create_shipment_service]) ) {
					...
					foreach ($shipmentIds as $shipment_id) {
						...
						if (!empty($shipping_label)) {
							...
							if ($this->settings['dir_download'] == 'yes') { 
								?>
								<a class="button button-primary tips label_buttons wf_australiapost_viewlabel" target="_self" href="<?php echo $get_label_url; ?>" data-tip="<?php _e('Download Label', 'wf-shipping-auspost'); ?>"><?php _e('Download Label', 'wf-shipping-auspost'); ?></a>
								<?php
						...
						<tbody class="table_body_packages_auspost_elex">
							...
							foreach ($shipment_requests as $shipment_request) {
								...
								<tr>
									...
									<?php } ?>
									<td align="left" class="shipment_description_row_columns"><input type="hidden" id="australia_post_package_manual_cubic_volume" name='elex_auspost_manual_cubic_volume[]' size="2" value="<?php echo ( ( isset($shipment_request['cubic_volume']) && $shipment_request['cubic_volume'] > 0 ) ? $shipment_request['cubic_volume'] : 0 ); ?>" /><input type="text" id="australia_post_package_manual_weight" name='elex_auspost_manual_weight[]' size="2" style="width: 100% !important" value="<?php echo round($shipment_request['Weight']['Value'], 2); ?>" />&nbsp;<?php echo 'kg'; ?></td>
									<td align="left" class="shipment_description_row_columns"><input type="text" id="australia_post_package_manual_length" name='elex_auspost_manual_length[]' size="2" class="shipment_row_columns_input_style" style="width: 100% !important" value="<?php echo round($shipment_request['Dimensions']['Length'], 2); ?>" />&nbsp;<?php echo 'cm'; ?></td>
									<td align="left" class="shipment_description_row_columns"><input type="text" id="australia_post_package_manual_width" name='elex_auspost_manual_width[]' size="2" class="shipment_row_columns_input_style" style="width: 100% !important" value="<?php echo round($shipment_request['Dimensions']['Width'], 2); ?>" />&nbsp;<?php echo 'cm'; ?></td>
									<td align="left" class="shipment_description_row_columns"><input type="text" id="australia_post_package_manual_height" name='elex_auspost_manual_height[]' size="2" class="shipment_row_columns_input_style" style="width: 100% !important" value="<?php echo round($shipment_request['Dimensions']['Height'], 2); ?>" />&nbsp;<?php echo 'cm'; ?></td>
									<td align="left" class="shipment_description_row_columns">
										<select class="select elex-auspost-package-service" id="australia_post_package_manual_service">';
											<?php 
											if (empty($postage_products_eligible) && empty($postage_products_eligible_startrack)) {
			...
			jQuery(document).ready(function() {
				...
				jQuery('.add_extra_packages').on('click', function(e) {
					...
					if (destinationCountry == 'AU') {
						...
						addExtraPackageHtml += '<td><img id="addPackageLoaderImage" src="<?php echo untrailingslashit(content_url('plugins/australia-post-woocommerce-shipping/australia_post/images/ELEX_AusPost_loader.gif')); ?>" style="width: 40%; height: 30%;" ></td>';
				...
				jQuery(document).on('click', '#auspost_generate_refresh_service_button', function(e) {
					e.preventDefault();
					var orderId = <?php echo json_encode($order_id); ?>;
					var rates_loader_img_html = `<img src=" <?php echo untrailingslashit(plugins_url()) . '/australia-post-woocommerce-shipping/australia_post/images/load.gif'; ?>"  align="center" style=" display:block;margin-left:auto;margin-right:auto;width:30%;" id="rates_loader_img" class="rates_loader_img">`;
...
<?php
	...
	public function elex_auspost_get_shipping_services( $package, $order_id) {
		...
		if (is_array($package['items'])) {
			...
			} elseif ($group_shipping_enabled == true && !$is_international) {
