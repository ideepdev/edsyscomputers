...
function transaction_report_admin_menu() {
    global $team_page;
    add_menu_page( __( 'Transaction Report', 'transaction-report' ), __( 'Transaction Report', 'transaction-report' ), 'edit_posts', 'transaction-report', 'transaction_report_admin_page', 'dashicons-format-aside', 8 ) ;
}
add_action( 'admin_menu', 'transaction_report_admin_menu' );
function transaction_report_admin_page(){
        $args = array(
            'exclude' => array(1),
        );
        ...
        <div class="main-content">  
            ...
            <form action="" method="post">  
                ...
                <div class="form-row">  
                    <label>  
                        ...
                        <?php 
                            foreach( $users as $user ){
                                ?>               
                                    <option value="<?php echo $user->ID ?>" <?php echo ((isset($_POST['user_id']) && in_array($user->ID, $_POST['user_id']) ? 'selected' : '')); ?> ><?php echo $user->user_login; ?></option>
                                <?php
                            }
                        ?> 
                        </select>  
                    ...
                    <label>  
                        <span>Date Form</span>  
                        <input type="date" name="date_from" value="<?php echo isset($_POST['date_from']) ? $_POST['date_from'] : ''; ?>">
                    </label> 
                    <label>  
                        <span>Date To</span>  
                        <input type="date" name="date_to" value="<?php echo isset($_POST['date_to']) ? $_POST['date_to'] : ''; ?>">
                    </label> 
                </div>     
                <div class="form-row">  
            ...
            <?php
                if(isset($_POST["user_id"]) && ($_SERVER["REQUEST_METHOD"] == "POST")){
                    ...
                    $initial_date = $_POST["date_from"];
                    $final_date = $_POST["date_to"];
            ...
            <table class="table" style="font-size: 14px;font-weight: 400;">
                <thead>
                  <tr>
                    <th>Buying Entity</th>
                    <th>Buying Contact Name</th>
                    <th>Buying Contact Phone</th>
                    <th>Buying Contact Email</th>
                    <th>Dealer Name</th>
                    <th>PO Number</th>
                    <th>Order Number</th>
                    <th>Order Date</th>
                        ...
                        if ( $shop_order ){
                            foreach ( $shop_order as $order ) {
                                ...
                                if ( $order_invoice && $order_invoice->exists() ) {
                                    ...
                                    $order_invoice_date = $order_invoice->get_date();
                                }else{
                                    $order_invoice_number = "";
                                    $order_invoice_date = "";
                                }
                                $order_delivery_address = $order->get_shipping_first_name()."\n".$order->get_shipping_last_name()."\n".$order->get_shipping_company()."\n".$order->get_shipping_address_1()."\n".$order->get_shipping_address_2()."\n".$order->get_shipping_city()."\n".$order->get_shipping_state()."\n".$order->get_shipping_postcode()."\n".$order->get_shipping_country();
                                $order_billing_postcode = $order->get_billing_postcode();
                                $order_total = $order->get_total();
                                $order_completed_dates = $order->get_date_completed();
                                foreach ($order->get_items() as $item_id=>$item) {
                                    ?>
                                    <tr>
                                    <?php
                                    ...
                                    if($product){
                                        ...
                                        $order_active_price = $product->get_price() - $remove_percentage_price;
                                    ...
                                    echo "<td>".$order_billing_company."</td>";
                                    echo "<td>".$order_user_login."</td>";
                                    echo "<td>".$order_billing_phone."</td>";
                                    echo "<td>".$order_billing_email."</td>";
                                    echo "<td>".'Edsys Computers'."</td>";
                                    echo "<td>".$order_purchase_number."</td>";
                                    echo "<td>".$order_so_order_number."</td>";
                                    echo "<td>".$order_date."</td>";
                                    echo "<td>".$order_invoice_number."</td>";
                                    echo "<td>".$order_invoice_date."</td>";
                                    echo "<td>".'Malaga'."</td>";
                                    echo "<td>".$order_completed_dates."</td>";
                                    echo "<td>".$order_delivery_address."</td>";
                                    echo "<td>".$order_billing_postcode."</td>";
                                    echo "<td>".$item_sku."</td>";
                                    echo "<td>".$order_serial_number."</td>"; 
                                    echo "<td>".''."</td>";
                                    echo "<td>".$order_active_price."</td>";
                                    echo "<td>".$order_item_quantity."</td>";
            ...
            <?php
                    ...
                    echo "Complete Record Download file: <b style=\"color:red;\"><a href='$FileName_csv' download='$FileName_csv'>Export to Csv Report</a></b>";
                    fclose($file_xls);
                    $admin_url = admin_url($FileName_xls);
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;<b style=\"color:red;\"><a href='$FileName_xls' download='$FileName_xls'>Export to Excel Report</a></b>";
